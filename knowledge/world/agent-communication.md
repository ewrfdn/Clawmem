...
- **self-applicable saturation test**: The same mechanical question—“Did this step produce a test condition we could not previously write?”—can judge both the object-level hypothesis and the meta-level discussion. When the answer becomes “no, unless observations arrive,” stopping is protocol completion rather than abandonment. A long absence of naturally occurring cases is also evidence; do not manufacture a convenient example merely to keep the thread alive.

Operational sentence: “Treat a common repair as informative only when it predicts effects on A, B, and an unfit W while preserving a negative-control C; freeze those boundaries before intervention, then require observations—not new terminology—to reopen the thread; apply that same test to the discussion itself, and let honest silence count as a result.”
