...
答：能商用，但有强 copyleft 义务。

核心点：

- 你可以商用、部署、收费。
- 如果你修改了 AGPLv3 程序，并通过网络提供服务，用户也有权获得对应修改版源码。
- 这比 GPL 更严格，因为 GPL 主要在“分发二进制”时触发，而 AGPL 把“网络服务”也纳入触发范围。

简单判断：

```text
内部自用、不对外提供服务：风险较低
修改后作为 SaaS/网络服务提供：通常需要开源修改版
只通过 API 调用未修改 AGPL 服务：要看集成方式和法律细节
```
...
## 38. Parallel Search 是什么？

**问：OpenClaw 当前 web_search provider 是什么？Parallel 是什么？**

答：当时检查到 OpenClaw 配置里没有显式 search provider，按 OpenClaw 文档默认使用 `parallel-free`，也就是 Parallel 免费 Search MCP。

Parallel 可以理解为 AI-native search / retrieval API：它不是传统浏览器搜索页面，而是面向 AI agent 的检索接口，返回更适合模型消费的结构化/摘要化内容。

---

## 39. OpenViking 是什么许可？

**问：OpenViking 的基本 GitHub 信息是什么？**

答：当时查询到：

- repo: `https://github.com/volcengine/OpenViking`
- created_at: `2026-01-05T07:11:17Z`
- earliest main commit: `2026-01-29T12:29:19Z`
- earliest release: `v0.1.9` at `2026-02-05T08:24:27Z`
- license: `AGPL-3.0`

因此如果要基于它做商业网络服务，需要特别关注 AGPLv3 的网络服务源码提供义务。

---

## 40. 这次 nginx/下载服务学到什么？

**问：这次打包 `claude-code` 并暴露下载链接有什么运维结论？**

答：

- `claude-code` repo 位于：`/home/azureuser/.openclaw/workspace/claude-code`
- remote：`https://github.com/instructkr/claude-code.git`
- 压缩包：`/home/azureuser/.openclaw/workspace/claude-code.tar.gz`
- SHA256：`c7b92f9101eec27e18d8f338a33100741cadd8f39eb5c35b4bd4606b246afbd9`
- 已复制到 `/var/www/sub/claude-code.tar.gz`
- 普通公网 URL：`https://bocchi.japaneast.cloudapp.azure.com/sub/claude-code.tar.gz`

用户明确偏好普通 URL 下载，不依赖 `MEDIA:` 附件渲染。
...
后续改 nginx 必须先检查生效文件，不能盲改 `sites-available`。

---

## 41. 抢票项目现在是什么状态？

**问：之前部署的抢票项目还在 nginx 配置里吗？**

答：当时确认还在。

信息：

- 项目路径：`/home/azureuser/.openclaw/workspace/biliTickerBuy`
- 监听：`127.0.0.1:7860`
- 公网路径：`https://bocchi.japaneast.cloudapp.azure.com/btb/`
- 有效 nginx 配置代理 `/btb/` 和 `/__btb/` 到 `127.0.0.1:7860`
- 本地和公网都验证过 `200 OK`
...
location ^~ /btb/ {
    proxy_pass http://127.0.0.1:7860;
    proxy_set_header X-Forwarded-Prefix /btb;
}
```
...
**问：把所有机制抽象成一句话，agent runtime 是什么？**

答：agent runtime 是一个围绕模型的状态机，不只是 LLM API wrapper。
...
真正的 agent 工程难点不在“让模型会 function calling”，而在 **如何让 runtime 长期、安全、可恢复、可压缩、可审计地承载这些调用**。
