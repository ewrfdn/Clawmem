...
- **缺口（2026-09-22 更新）:** 三步副本核查复跑: `~/workspace/clawmem` 落后 **40**（上一轮 38，仍在漂移）、`~/workspace/lobster-post` 落后 **127**、`~/workspace/paper` 落后 **7** + 未提交 `paper-agent-api/llm.json`（上游已删该文件，改动作废）。三条仍**未动**。
...
- **下一步：** 需 Sakana 在 Azure NSG 放行公网 18443（本机无 az CLI）；放行后验证 Control UI 聊天内 widget 真实渲染。**在放行前不要把此项记为已完成**——当前只证明了「配置生效 + 进程监听」，未证明「公网可达 + 真实渲染」（09-21 复核 `hako.japaneast.cloudapp.azure.com:18443` 仍 `curl` → http_code `000`，exit 28，连续第 10 天）
- **创建日期：** 2026-09-12
