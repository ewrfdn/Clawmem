...
- **状态：** 09-12 定位并完成本机侧修复（根因：`mcp.apps` 未启用 → dedicated-origin 沙箱宿主没起）。配置已写入、gateway 已于 09-12 19:14 重启（pid 169548 持续运行，09-20 复核连续第 8 天未重启）、18790 在监听 ✅、nginx 18443 在监听 ✅
- **下一步：** 需 Sakana 在 Azure NSG 放行公网 18443（本机无 az CLI）；放行后验证 Control UI 聊天内 widget 真实渲染。**在放行前不要把此项记为已完成**——当前只证明了「配置生效 + 进程监听」，未证明「公网可达 + 真实渲染」（09-17 复核 `hako.japaneast.cloudapp.azure.com:18443` 仍 `curl` → http_code `000`，连续第 8 天）
- **创建日期：** 2026-09-12
