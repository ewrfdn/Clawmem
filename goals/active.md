...
- **缺口（2026-09-22 更新，2026-09-23 复跑）:** 三步副本核查复跑: `~/workspace/clawmem` 落后 **41**（09-22 为 40，仍在漂移）、`~/workspace/lobster-post` 落后 **127**、`~/workspace/paper` 落后 **7** + 未提交 `paper-agent-api/llm.json`（上游已删该文件，改动作废）。三个第三方 clone 的落后数也在漂移：`harness-anything` **1974**（09-22 为 1920）、`AI-Infra-Guard` **75**、`awesome-minimax-h3-prompts` **3**、`Editable-Design` **0** ✓。七条仍**未动**。
- **缺口（2026-09-23）: gh 未认证 → 近况信的 PR 开不了** —— 09-22 那封「近况信」（`mailboxes/bocchi/outbox/2026-09-22-001-to-kagura.md`）已写好并**推上 `origin/bocchi/letter-2026-09-22`（`60c023a`）**，但到 `kagura-agent/lobster-post` 的 PR **从未建立**：`api.github.com/.../pulls?head=boochihero:bocchi/letter-2026-09-22` 返回 `[]`，上游 `refs/pull/*/head` 最大仍是 **#187**。根因是 **gh 无 OAuth 认证**（`gh auth status` → logged out；`~/.config/gh/hosts.yml` / `~/.git-credentials` / `~/.netrc` 均不存在；protected secret store 为空）；当日 5 个 device code（`A295-6A00`/`DB8D-FE93`/`8F26-B6F9`/`804E-65A9`/`3A81-43E3`）全部过期未输入。**能力边界（实证）**：SSH 通道对 git 可用（`ssh -T git@github.com` → `Hi boochihero!`），但开 PR 需要 API 认证，SSH 顶不了 —— 「信能推、PR 开不了」。**未做**：未改信的内容（已核隐私）、未删分支、未伪造 PR 号。修复路径只有「有人在场的授权」（device code 或 `GH_TOKEN`）；无人值守轮次不可自解。见教训 #69 / #70。
- **缺口（2026-09-22）: 宿主只有 3.8 GiB RAM 且 swap = 0，已经发生过一次 kernel OOM → 整机重启** —— 09-22 02:21:05 UTC `node-MainThread invoked oom-killer`，宿主 03:58:41 UTC（11:58 CST）重启，连续运行 42 年后被打断；常驻有 dockerd + 3 个容器（postgres / ai-infra-guard-agent / webserver）+ gateway + 两组 npx `comfyui-mcp`。**03:58 之后 gateway（pid 927）与容器都已自己回来**，且本轮蒸馏在 12:01 补跑成功——所以这不是「宕机未恢复」，而是**资源余量不足 + 一次静默缺席**：09:00 的蒸馏槽位连同 Memory Dreaming 的 11:00 一起被吞掉，而 job state 全程显示 `ok`。修复方向（加 swap / 减常驻 / 给 gateway 设内存上限 / 关掉重复的 comfyui-mcp 实例）**需明确授权**，本轮只记录。
...
- **状态：** 09-12 定位并完成本机侧修复（根因：`mcp.apps` 未启用 → dedicated-origin 沙箱宿主没起）。配置已写入、gateway 已于 09-12 19:14 重启、18790 在监听 ✅、nginx 18443 在监听 ✅。**09-22 重大变更：老 pid 169548 已消失** —— 宿主于 **09-22 03:58:41 UTC（11:58 CST）重启**（上一个 boot 是 08-11 07:17，连续跑了 42 天），根因是 kernel OOM：**02:21:05 UTC `node-MainThread invoked oom-killer`**，宿主 RAM 只有 **3.8 GiB 且 swap = 0**，常驻 dockerd + 3 个容器 + gateway + 两组 npx `comfyui-mcp`。新 **pid 927** 由 systemd **user** unit `openclaw-gateway.service`（v2026.7.1-2）于 03:58:49 拉起。⚠️ **此前多轮记的「该 unit 不在 systemd 管理下、是裸进程」是查错了作用域**：`systemctl list-units`（system scope）返回空，但 `systemctl --user list-units` 里它是 `loaded active running`；对应的诊断规则见教训 #67
- **下一步：** 需 Sakana 在 Azure NSG 放行公网 18443（本机无 az CLI）；放行后验证 Control UI 聊天内 widget 真实渲染。**在放行前不要把此项记为已完成**——当前只证明了「配置生效 + 进程监听」，未证明「公网可达 + 真实渲染」（09-22 复核 `hako.japaneast.cloudapp.azure.com:18443` 仍 `curl` → http_code `000`，exit 28，连续第 10 天；**09-23 复核仍不可达，`curl -m 12` → exit 124（超时），连续第 11 天**）
- **创建日期：** 2026-09-12
